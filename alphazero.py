...
from helpers import (
    argmax, check_space, is_atari_game, copy_atari_state, store_safely,
...
class Model:
    ...
    def __init__(self, Env, lr, n_hidden_layers, n_hidden_units):
        ...
        if not self.state_discrete:
            self.x = x = tf.placeholder("float32", shape=np.append(None, self.state_dim),
                                        name='x')  # state
        else:
            self.x = x = tf.placeholder("int32", shape=np.append(None, 1))  # state
            x = tf.squeeze(tf.one_hot(x, self.state_dim, axis=1), axis=2)

        # Feedforward: Can be modified to any representation function, e.g. convolutions,
        # residual networks, etc.
...
class State():
    ...
    def select(self, c=1.5):
        ...
        UCT = np.array(
            [child_action.Q + prior * c * (np.sqrt(self.n + 1) / (child_action.n + 1)) for
             child_action, prior in zip(self.child_actions, self.priors)])
        winner = argmax(UCT)
    ...
    def evaluate(self):
        ...
        self.V = np.squeeze(
            self.model.predict_V(self.index[None,])) if not self.terminal else np.array(0.0)
...
class MCTS():
    ...
    def search(self, n_mcts, c, Env, mcts_env):
        ...
        if self.root is None:
            self.root = State(self.root_index, r=0.0, terminal=False, parent_action=None,
                              na=self.na, model=self.model)  # initialize new root
        else:
...
#### Agent ##
def agent(game, n_ep, n_mcts, max_ep_len, lr, c, gamma, data_size, batch_size, temp,
    ...
    with tf.Session() as sess:
        ...
        for ep in range(n_ep):
            ...
            if R > R_best:
                ...
                seed_best = seed
                R_best = R
            print('Finished episode {}, total return: {}, total time: {} sec'.format(ep,
                                                                                     np.round(R, 2),
                                                                                     np.round((
                                                                                             time.time() - start),
                                                                                         1)))
...
if __name__ == '__main__':
    ...
    episode_returns, timepoints, a_best, seed_best, R_best = agent(game=args.game, n_ep=args.n_ep,
                                                                   n_mcts=args.n_mcts,
    ...
    ax.plot(symmetric_remove(np.arange(total_eps), args.window - 1), episode_returns, linewidth=4,
            color='darkred')
